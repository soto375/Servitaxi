  ...
  function actualizarVistaVideo(){
    ...
    if(archivosVideo.length == 0){
      ...
      for(const archivo of archivosVideo) {
        ...
        lista.className ="col-sm-4 embed-responsive embed-responsive-16by9 mr-2";
  ...
  function actualizarVistaImagen(){
    ...
    if(archivos.length == 0){
      ...
      for (const archivo of archivos) {
        const lista = document.createElement("div");
        lista.className ="col-sm-2";
        previsualizacion.appendChild(lista);
        const ele = document.createElement('p');
        if(validarTipoArchivo(archivo)) {

          nombreArchivo = (archivo.name.length>10)?archivo.name.substr(0,7)+"...":archivo.name;
          ele.textContent = `Nombre de archivo ${nombreArchivo}, tamaño de imagen ${tamanhoArchivo(archivo.size)}.`;
