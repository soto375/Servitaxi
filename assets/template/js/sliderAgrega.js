$(document).ready(function(){
  ...
  form.addEventListener('submit', function(event){
    ...
    if(!(tamanhoDescripcion.length)){
      ...
      errorDescripcion.textContent = "El campo Descripción de slider es requerido.";
      validado = false;
    }else if((tamanhoDescripcion.length<3)){
       $(".descripcion").addClass("has-error");
      errorDescripcion.textContent = "El campo Descripción de slider debe contener mínimo 3 carácteres.";
      validado = false;
    }else{
    ...
    if(!(imagen.files.length)){
      ...
      errorImagen.textContent = "El campo Imagen de portada es requerido.";
      error = false;
    }else if(!(imagen.files[0].size/1048576 < 5)){
      $(".imagenFile").addClass("has-error");
      errorImagen.textContent = "El campo Imagen de portada supera el tamaño máximo permitido de 5 MB.";
      error = false;
    }else{
